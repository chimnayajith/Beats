...
const { getRestricted } = require("../utils/restrictUtil")
module.exports = async (client, interaction) => {
  const dmEmbed =  new EmbedBuilder().setColor("#2f3136").setTitle("Hey there!")
    .setDescription(`Commands do not work in the DM's.We kindly request that you join our support server using the button provided below for any assistance or inquiries regarding the bot. This support server serves as the official channel to receive support, as well as stay updated with the latest updates and announcements related to the bot. Thank you for understanding.`)
  ...
  if (interaction.channel.isDMBased()) return interaction.reply({embeds : [dmEmbed] , components : [supportButton]});
  ...
  const row = new ActionRowBuilder()
    ...
    if (cmd && cmd.vote) {
      ...
      patron : if ( user_plus.includes(interaction.user.id) || guild_plus.includes(interaction.guild.ownerId) || guild_plus.includes(interaction.user.id)  ) {
        break patron;
      }
      else {
    ...
    if (data) {
      const restricted = new EmbedBuilder().setColor("#2f3136").setDescription(`You have been restricted from using Beats commands. Use commands again <t:${Math.floor(data.expireAt.getTime() / 1000)}:R>`).setTitle("Restricted")
      .addFields(
        {
    ...
    if (cmd) {
      if (interaction.commandName == cmd.data.name) {
        ...
        logger.info(`Command: ${cmd.data.name}, Guild ID: ${interaction.guild.id}, User ID: ${interaction.user.id}, Arguments: ${JSON.stringify(interaction.options._hoistedOptions)}`)
