...
module.exports = async (queue, track) => {
  await logIfRequired(
    queue.options.guild.id,
    queue.options.guild.ownerId,
    "playLogs",
    {
  ...
  const likeSuccess = new EmbedBuilder()
    .setColor("#2f3136")
    .setTitle(`${track.title} Liked`)
    .setURL("https://dashboard.beatsbot.in/likedsongs")
    .setDescription(
      "Track has been added to your liked songs. Use the command `/liked` to play your liked songs!",
    )
    .setThumbnail("https://cdn.beatsbot.in/attachments/favourites.png");
  const dupeTrack = new EmbedBuilder()
    .setColor("#2f3136")
    .setDescription(
      `<a:warn:889018313143894046> ⠀|⠀ This track is already liked by you!`,
    );
  ...
  if (
    ...
    const newTrack = await queue.metadata.interaction.channel.send({
      embeds: [embed1],
      components: [row],
      fetchReply: true,
    });
    ...
    collector.on("collect", async (collected) => {
      ...
      if (collected.customId === "like") {
        ...
        const spotifyExtractor = player.extractors.get(
          SpotifyExtractor.identifier,
        );
        const relatedTracks = track.extractor
          ? await track.extractor.getRelatedTracks(track)
          : await spotifyExtractor.getRelatedTracks(track);
        ...
        const searchEmbed = new EmbedBuilder()
          ...
          .setDescription(
            `\n\n${relatedTracks.tracks
              .map(
                (t, i) => `**${i + 1}**) [${t.title} - ${t.author}](${t.url})`,
              )
              .slice(0, 10)
        ...
        const mOptions = relatedTracks.tracks.slice(0, 10).map((each, i) => ({
          label: each.title.substr(0, 90),
          value: i.toString(),
        }));
        ...
        const msg = await collected.followUp({
          embeds: [searchEmbed],
          components: [mRow, cRow],
          fetchReply: true,
        });
        ...
        collector.on("collect", async (collected) => {
          ...
          if (collected.isStringSelectMenu()) {
            const embed2 = new EmbedBuilder()
              .setColor("#2f3136")
              .setDescription("You should be in the listening party");
            if (
              !collected.member.voice.channel ||
              (collected.guild.members.me.voice.channel &&
                collected.member.voice.channel.id !==
                  collected.guild.members.me.voice.channel.id)
            ) {
              await collected.deferUpdate();
            ...
            player.play(
              collected.member.voice.channel.id,
              relatedTracks.tracks[switchint],
              {
