...
module.exports = async (client, guild) => {
  await stats.updateOne({ _id: "guilds" }, { $push: { guild_ids: guild.id } });
  ...
  const channel =
    (await guild.systemChannel) || (await getDefaultChannel(guild));

  channel.send({ embeds: [newGuild] }).catch(console.error);

  const webhookClient = new WebhookClient({
    id: "1118369501679976479",
    token:
      "urX29dG8RvTL_urvJTiW0CCOztoziRCyLo-Pe_PqeR8ThUU4786otU28zpqUk5qAjmFJ",
  });
