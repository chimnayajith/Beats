    ...
    data.forEach((document) => {
      ...
      if (currentDate > nextVoteDate) {
        const embed = new EmbedBuilder()
          .setColor("#2f3136")
          .setDescription(
            "It's been 12 hours since your last vote for Beats on DBL (Discord Bot List). Don't miss out on the opportunity to unlock all vote-restricted commands and support the growth of Beats.",
          )
        ...
        client.users.fetch(document.userId).then((voter) => {
          voter.send({ embeds: [embed], components: [row] }).catch(() => {});
