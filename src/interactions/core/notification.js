...
module.exports = {
  ...
  async execute(client, interaction) {
    ...
    const noNotifs = new EmbedBuilder()
      .setColor("#2f3136")
      .setDescription(
        "<a:warn:889018313143894046>⠀ | ⠀No notifications currently!",
      );
    ...
    function paginateNotifs() {
      notifEmbed
        .setDescription(data[current])
        .setFooter({ text: `Page : ${current + 1}/${data.length}` });
    }
    ...
    if (data.length === 0) {
      ...
      const notif =
        data.length > 1
          ...
          : await interaction.reply({ embeds: [notifEmbed], fetchReply: true });
