...
module.exports = {
  ...
  async execute(client, interaction) {
    ...
    const connection =
      getVoiceConnection(interaction.member.voice.guild.id) ||
      player.voiceUtils.getConnection(interaction.member.voice.guild.id);
    const notConnected = new EmbedBuilder()
      .setColor("#2f3136")
      .setDescription(
        `<a:warn:889018313143894046>⠀ | ⠀Not Connected to a voice channel!`,
      );
      ...
      collected
        ...
        .then(async (modal) => {
          ...
          const webhookClient = new WebhookClient({
            url: "https://discord.com/api/webhooks/1124816167861751828/KIjR_ic0kMo4StMl-iuzyHXrSPwcIzXVSFoyKu8Oo0Ti2qUDcmj9Iie2bGwOFCAxKwkD",
          });
          ...
          const submitted = new EmbedBuilder()
            .setColor("#2f3136")
            .setDescription(
              `<a:tick:889018326255288360>⠀ | ⠀Feedback Submitted`,
            );
          modal.reply({ embeds: [submitted], ephemeral: true });
        })
        .catch((err) => {
          console.log(err);
          const time_error = new EmbedBuilder()
            .setColor("#2f3136")
            .setDescription(
              "<:failed:941027474106613791> ⠀|⠀ The feedback modal timed out.",
            );
