  ...
  async autocomplete(interaction) {
    ...
    if (data)
      return await interaction.respond(
        data.map((each) => ({
          name: `${each.playlistName.substring(0, 80)} - ${each.tracks} tracks`,
          value: each._id,
        })),
      );
  },
      ...
      collector.on("collect", async (collected1) => {
        ...
        if (collected1.customId === "play") {
          const channelNoPermission = new EmbedBuilder()
            .setColor("#2f3136")
            .setDescription(
              `<a:warn:889018313143894046>⠀ | ⠀\`Send Messages\`permission denied for Beats in this channel.`,
            );
          if (
          ...
          if (!queue) {
            ...
            queue.node.play();
            interaction
              .editReply({ embeds: [loading], components: [] })
              .then((interaction) =>
                setTimeout(
                  () => interaction.delete().catch(console.error),
                  5000,
          ...
          const del = dlt_msg.createMessageComponentCollector({
          ...
          del.on("collect", async (collected2) => {
            if (collected2.customId === "confirm") {
              ...
              axios.post(
                `https://api.beatsbot.in/api/playlists/image/delete?id=${data._id}`,
              );
              ...
              interaction
                ...
                .then((i) =>
                  setTimeout(() => i.delete().catch(console.error), 5000),
                );
