...
module.exports = {
  ...
  async execute(client, interaction) {
    ...
    const noTime = new EmbedBuilder()
      .setColor("#2f3136")
      .setDescription(
        `**:bangbang:⠀ | ⠀Specify position to seek**\n\`\`\`fix\n Use "/seek 2s"  , "/seek 1m" etc\`\`\``,
      );
    if (!query) return interaction.reply({ embeds: [noTime] });

    const timeToMS = ms(query);

    const couldntSeek = new EmbedBuilder()
      .setColor("#2f3136")
      .setTitle(":x: ⠀|⠀ Time could not be seeked!")
      .setDescription(`\`\`\`yaml\nUse format /seek 2s or 1m\`\`\``);
    if (!timeToMS)
      return interaction.reply({ embeds: [couldntSeek], ephemeral: true });

    const invalidTime = new EmbedBuilder()
      .setColor("#2f3136")
      .setTitle(
        `<a:warn:889018313143894046>⠀ | ⠀Provide a valid time below total time.`,
      )
      .setDescription("```yaml\nUse format /seek 2s or 1m```");
    if (timeToMS >= track.durationMS)
    ...
    const seekSuccess = new EmbedBuilder()
      .setColor("#2f3136")
      .setTitle(
        `<a:tick:889018326255288360> ⠀|⠀ Time set to **${prettyMs(timeToMS, { colonNotation: true })}**`,
      )
      .setDescription(
