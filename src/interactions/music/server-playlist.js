  ...
  async autocomplete(interaction) {
    ...
    if (data)
      return await interaction.respond(
        data.map((each) => ({
          name: `${each.playlistName.substring(0, 85)} - ${each.playlistTracks.length} tracks`,
          value: each._id,
        })),
      );
  },
  async execute(client, interaction) {
    if (interaction.options.getSubcommand() === "setup") {
      ...
      const no_perm = new EmbedBuilder()
        .setColor("#2f3136")
        .setDescription(
          "<a:warn:889018313143894046>⠀ | ⠀You need the `Manage Guild` permission to create server playlist!",
        );
      if (!interaction.member.permissions.has(PermissionFlagsBits.ManageGuild))
        return interaction.reply({ embeds: [no_perm], ephemeral: true });

      //server already has 2 playlists!
      const maxPlaylist = new EmbedBuilder()
        .setColor("#2f3136")
        .setDescription(
          `<a:warn:889018313143894046>⠀ | ⠀ Server already has 2 server playlists.`,
        );
      if (data.length === 2)
        return interaction.reply({ embeds: [maxPlaylist], ephemeral: true });
      ...
      await logIfRequired(
        ...
        "playlistUpdateLogs playlistSetup",
        {
      ...
      const playlistCreated = new EmbedBuilder()
        ...
        .addFields(
          {
            name: "Playlist Name",
            value: `\`${newdata.playlistName}\``,
            inline: true,
          },
          {
            name: "Playlist Manager Role",
            value: `<@&${newdata.roleID}>`,
            inline: true,
          },
          {
            name: "\u200B",
            value: `Use </server-playlists add:1126830641154109560> to add songs to the playlist`,
            inline: false,
          },
        )
      ...
      const noPlaylists = new EmbedBuilder()
        .setColor("#2f3136")
        .setDescription(
          `<a:warn:889018313143894046>⠀ | ⠀Server doesn't have any playlist. Use \`/server-playlists setup\` to create a playlist!`,
        );
      if (!serverplaylists)
      ...
      const invalidPlaylist = new EmbedBuilder()
        .setColor("#2f3136")
        .setDescription(
          `<a:warn:889018313143894046>⠀ | ⠀Choose from an existing playlist!`,
        );

      //not object id
      if (!mongoose.Types.ObjectId.isValid(playlistId))
        return interaction.reply({
          embeds: [invalidPlaylist],
          ephemeral: true,
        });
      ...
      const notManager = new EmbedBuilder()
        .setColor("#2f3136")
        .setDescription(
          `<a:warn:889018313143894046>⠀|⠀You need the <@&${data.roleID}> role to add songs to \`${data.playlistName}\``,
        );
      if (!interaction.member.roles.cache.has(data.roleID))
        return interaction.reply({ embeds: [notManager], ephemeral: true });
      ...
      const noMusic = new EmbedBuilder()
        .setColor("#2f3136")
        .setDescription(
          `<a:warn:889018313143894046> ⠀|⠀ No track is playing to add to playlist!`,
        );
      ...
      await logIfRequired(
        interaction.guild.id,
        interaction.guild.ownerId,
        "playlistUpdateLogs playlistAdd",
        {
          guildName: interaction.guild.name,
          guildID: interaction.guild.id,
          guildIcon: interaction.guild.iconURL(),
      ...
      const addedToPlaylist = new EmbedBuilder()
        .setColor("#2f3136")
        .setDescription(
          `[${queue.currentTrack.title}](${queue.currentTrack.url}) added to server playlist : ${data.playlistName}.\n\nUse </server-playlists play:1126830641154109560> to play songs in the playlist!`,
        )
      ...
      const noTracks = new EmbedBuilder()
        .setColor("#2f3136")
        .setDescription(
          `<a:warn:889018313143894046> ⠀|⠀ No tracks added to \`${data.playlistName}\`. Use </server-playlists add:1126830641154109560> to add songs to the playlist`,
        );
      ...
      if (
        ...
        return interaction.reply({ embeds: [diffVoice], ephemeral: true });
      ...
      const playlistLoading = new EmbedBuilder()
        .setColor("#2f3136")
        .setDescription(
          `<a:loading:889018179471441941>⠀ | ⠀Loading your Server Playlist : ${data.playlistName}`,
        );
      const queue = player.nodes.get(interaction.guild.id);
      const tracks = [];
      ...
      if (!queue) {
        ...
        queue.node.play();
        interaction
          .reply({ embeds: [playlistLoading] })
          .then((interaction) =>
            setTimeout(() => interaction.delete().catch(console.error), 5000),
          );
      } else {
        interaction
          .reply({ embeds: [playlistLoading] })
