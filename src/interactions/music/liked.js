...
module.exports = {
  ...
  async execute(client, interaction) {
    ...
    const noLiked = new EmbedBuilder()
      .setColor("#2f3136")
      .setDescription(
        "<a:warn:889018313143894046> ⠀|⠀ You have no Liked Songs!",
      );
    if (data === null)
      return interaction.reply({ embeds: [noLiked], ephemeral: true });
    ...
    if (!interaction.member.voice.channel.joinable)
      return interaction
        ...
        .then((interaction) =>
          setTimeout(() => interaction.delete().catch(console.error), 10000),
        );
    joinVoiceChannel({
    ...
    const loadingLiked = new EmbedBuilder()
      .setColor("#2f3136")
      .setDescription(
        `<a:loading:889018179471441941>⠀ | ⠀Loading your \`Liked Songs (${data.length} tracks).\``,
      );
