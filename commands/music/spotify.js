...
const { EmbedBuilder, ButtonBuilder, StringSelectMenuBuilder , ActionRowBuilder , WebhookClient ,PermissionFlagsBits} = require("discord.js");
...
module.exports = {
  ...
  async execute(client, interaction) {
    if (interaction.options.getSubcommand() === "connect") {
        ...
        const noPlaylists = new EmbedBuilder().setColor("#2f3136").setDescription("You do not have any public playlists!");
        
        if (! (playlistData.length > 0 )) return interaction.reply({embeds : [noPlaylists] , ephemeral : true});
        const availablePlaylists = []
        const pages = []
        const playlistsEmbed = new EmbedBuilder().setColor("#2f3136").setTitle(`<:spotify:1123967292900900965> Spotify Playlists Test: ${interaction.user.username}`).setThumbnail(`https://media.discordapp.net/attachments/889016433252634657/1124046362615500820/306f6a14403921a4d8b4ab53d3c9f2a3.png`);
        ...
        const playlistInfo = playlistData
            .filter((playlist) => playlist.public || playlist.owner.uri === 'spotify:user:spotify')
            .map((playlist, i) => {
                availablePlaylists.push(playlist)
                const { external_urls, name, tracks } = playlist;
                const trackCount = tracks.total.toString();
                const playlistName = name.length > 22 ? name.substring(0, 19) + '...' : name;
                availableOptions++
        ...
        collector.on("collect", async (collected) => {
            if(collected.componentType === 2){
                ...
                // Search playlist!
                const query = availablePlaylists[parseInt(collected.values[0])].external_urls.spotify
                ...
                if (!searchResult || !searchResult.tracks.length) return collected.editReply({ embeds: [no_result] ,components : [], ephemeral: true }).then((message) => setTimeout(() => message.delete().catch(console.error), 20000));
                ...
                joinVoiceChannel({
                  ...
                  const loadingTrack = new EmbedBuilder().setColor("#2f3136").setDescription(`<a:loading:889018179471441941>⠀ | ⠀Loading your playlist : [${availablePlaylists[parseInt(collected.values[0])].name}](${availablePlaylists[parseInt(collected.values[0])].external_urls.spotify})`);
                  collected.editReply({embeds : [loadingTrack] ,components : []}).then((message) => setTimeout(() => message.delete().catch(console.error), 20000))
            }
        });
    }
  },
